#> reus:manager/0_init/3_general/hp
#
# 火竜 召喚処理 HP・部位耐久値初期化
...
# HPセット
    scoreboard players set #mhdp_reus_health AsaMatrix 800000
    # HP倍率適用
    ...
    # 代入
        execute store result score #mhdp_reus_health_half AsaMatrix run scoreboard players operation #mhdp_reus_health AsaMatrix /= #asam_const_100 AsaMatrix
    # 体力半分取得
...
# 部位耐久値
    ## 頭：8%
    scoreboard players set #mhdp_const_temp AsaMatrix 8
...
# 怒り
    ## 12%
    scoreboard players set #mhdp_const_temp AsaMatrix 12
    scoreboard players operation #mhdp_reus_anger_damage AsaMatrix = #mhdp_reus_health AsaMatrix
    scoreboard players operation #mhdp_reus_anger_damage AsaMatrix *= #mhdp_const_temp AsaMatrix
