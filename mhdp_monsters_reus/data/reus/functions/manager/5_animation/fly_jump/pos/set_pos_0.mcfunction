...
execute rotated ~ 0 positioned as @e[tag=ReusAttackTarget] positioned ^ ^ ^ run tp @e[type=marker,tag=ReusMovePos] ~ ~1 ~
execute as @e[type=marker,tag=ReusMovePos] at @s if block ~ ~-0.3 ~ #asa_animator:no_collision run function asa_animator:general/check_ground
execute as @e[type=marker,tag=ReusMovePos] at @s run tp @s ~ ~0.5 ~
...
scoreboard players set #reus_height_target AsaMatrix 16
scoreboard players operation #reus_mine_x AsaMatrix /= #reus_height_target AsaMatrix
scoreboard players operation #reus_mine_y AsaMatrix /= #reus_height_target AsaMatrix
scoreboard players operation #reus_mine_z AsaMatrix /= #reus_height_target AsaMatrix
