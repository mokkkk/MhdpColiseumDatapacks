#> ranposu:manager/0_init/3_general/hp
#
# 青鳥竜 召喚処理 HP・部位耐久値初期化
...
# HPセット
    scoreboard players set #mhdp_ranposu_health AsaMatrix 500000
    # HP倍率適用
        scoreboard players operation #mhdp_ranposu_health AsaMatrix *= #mhdp_hp_multiply AsaMatrix
        scoreboard players operation #mhdp_ranposu_health AsaMatrix /= #asam_const_100 AsaMatrix
    # マルチ補正計算
        ...
        scoreboard players operation #mhdp_ranposu_health AsaMatrix *= #mhdp_const_temp_0 AsaMatrix
    # 代入
        execute store result score #mhdp_ranposu_health_half AsaMatrix run scoreboard players operation #mhdp_ranposu_health AsaMatrix /= #asam_const_100 AsaMatrix
    # 体力半分取得
        scoreboard players set #mhdp_const_temp AsaMatrix 2
        scoreboard players operation #mhdp_ranposu_health_half AsaMatrix /= #mhdp_const_temp AsaMatrix

# ボスバー設定
    bossbar add asa_animator:ranposu_health [{"text": "青鳥竜"},{"text":"\uE000","font":"mhdp"}]
    execute if data storage mh_dp:settings Custom{QuestRank:1} run bossbar set asa_animator:ranposu_health color red
    execute store result bossbar asa_animator:ranposu_health max run scoreboard players get #mhdp_ranposu_health AsaMatrix
    bossbar set asa_animator:ranposu_health players @a
    bossbar set asa_animator:ranposu_health visible true
...
# 怒り
    ...
    scoreboard players operation #mhdp_ranposu_anger_damage AsaMatrix = #mhdp_ranposu_health AsaMatrix
