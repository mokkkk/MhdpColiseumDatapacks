...
# 余ったアイテムはチェストに送還(252 66 222)
    ...
    execute if entity @s[tag=TmpLimitItemCount] run loot insert 252 66 222 loot mhdp_core:consumable_items/reset/52_adamantseed

# アイテム変換
    execute if entity @s[tag=TmpLimitItemCount] run scoreboard players set #mhdp_temp_item_count MhdpCore 10
    loot give @s loot mhdp_core:consumable_items/replace/52_adamantseed
