#> mhdp_core:player/damage/knockback/voice/0
#
# プレイヤーノックバック開始処理
...
# スキル確認
    execute if entity @s[tag=SklEarplugsLv1] run tag @s add NotKnockback
...
# スキルで無効化されていない場合，怯み処理実行
    execute unless entity @s[tag=NotKnockback] run function mhdp_core:player/damage/knockback/voice/0_
