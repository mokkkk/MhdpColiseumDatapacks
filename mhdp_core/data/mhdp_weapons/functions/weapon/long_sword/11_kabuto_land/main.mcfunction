...
# 攻撃
    # 練気が足りない場合，失敗
        execute if entity @s[scores={MhdpWeaponTimer=1,MhdpWeaponSpiritGaugeLsword=..299}] at @s run function mhdp_weapons:weapon/long_sword/10_kabuto/attack_miss
    # 練気が半分以上の場合，攻撃用Marker召喚
        execute if entity @s[scores={MhdpWeaponTimer=1,MhdpWeaponSpiritGaugeLsword=300..}] at @s run function mhdp_weapons:weapon/long_sword/10_kabuto/attack
