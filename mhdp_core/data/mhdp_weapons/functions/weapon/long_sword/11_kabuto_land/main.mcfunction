#> mhdp_weapons:weapon/long_sword/11_kabuto_land/main
#
# 太刀：気刃兜割
...
# 攻撃
    # 練気が満タンでない場合，失敗
        execute if entity @s[scores={MhdpWeaponTimer=1,MhdpWeaponSpiritGaugeLsword=..599}] at @s run function mhdp_weapons:weapon/long_sword/10_kabuto/attack_miss
    # 練気が満タンの場合，攻撃用Marker召喚
        execute if entity @s[scores={MhdpWeaponTimer=1,MhdpWeaponSpiritGaugeLsword=600..}] at @s run function mhdp_weapons:weapon/long_sword/10_kabuto/attack
