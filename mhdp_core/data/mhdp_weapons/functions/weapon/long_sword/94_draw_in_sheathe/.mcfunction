#> mhdp_weapons:weapon/long_sword/94_draw_in_sheathe/
#
# 太刀の特殊抜刀処理
#
...
# B:メインハンドにサブ武器がある
    execute if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].PlayerData.Item.MainHand.tag{MhdpWeaponSub:1b} run tag @s add FlagB

# C:ステップ回避中でない
    execute unless entity @s[tag=PlySneakAvoidFunc] run tag @s add FlagC

# A && B && Cの場合，抜刀成功
    execute if entity @s[tag=FlagA,tag=FlagB,tag=FlagC] run function mhdp_weapons:weapon/long_sword/94_draw_in_sheathe/draw
